...
<template>
  ...
  <div class="url-codec">
    <!-- 顶栏：标题 + 操作按钮 -->
    <header>
      <h1><span class="logo">🔗</span> URL 在线转码/解码</h1>
      <div class="toolbar">
        <button class="primary" @click="submit('encode')">转码</button>
        <button @click="submit('decode')">解码</button>
    ...
    <main ref="mainRef">
      ...
      <section class="editor" :style="paneStyle">
        ...
        <div class="config">
          <textarea v-model="input" class="input-area" spellcheck="false" placeholder="请输入待转码或解码的 URL/文本"></textarea>
          <!-- 错误提示：解码失败等异常信息 -->
      ...
      <section class="preview">
        ...
        <div class="preview-scroll">
          ...
          <div v-else class="empty-hint">输入内容后点击「转码」或「解码」即可获得结果</div>
        </div>
      </section>
    </main>
...
/* ---------- 载入示例 ---------- */
function loadSample() {
  input.value = site.devnote
  status.value = '已载入示例'
}

// 默认载入示例：页面打开时输入区预填示例 URL
loadSample()
...
function submit(mode) {
  ...
  try {
    if (mode === 'encode') {
      ...
      status.value = '转码成功 ✓ ' + new Date().toLocaleTimeString()
    } else {
      output.value = decodeURIComponent(input.value)
      status.value = '解码成功 ✓ ' + new Date().toLocaleTimeString()
    }
  } catch (e) {
...
/* ---------- 互换输入输出 ---------- */
// 把结果回填到输入区、清空结果，便于往返验证转码/解码
function swap() {
  if (!output.value) return
  input.value = output.value
  output.value = ''
  status.value = '已互换输入输出，可点击转码/解码'
}
...
@media (max-width: 734px) {
  ...
  .editor {
    ...
    border-bottom: 1px solid var(--border);
    width: 100% !important;
    /* 覆盖内联 width 的优先级，输入区占满屏幕宽度 */
    flex: none;
    min-height: 0;
    /* 高度由内联 style 控制（topHeight%） */
  }
